...
export default function ProblemCard({ 
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border-2 p-6 mb-6 transition-all duration-200 ${getCardBorderStyle()}`}>
      {/* Problem Header */}
      ...
      {problem.type === 'multiple_choice' && problem.options ? (
        ...
        <div>
          <label htmlFor={`input-${problem.id}`} className="block text-sm font-medium text-gray-700 mb-2">
            Your answer:
          </label>
          <input
            id={`input-${problem.id}`}
      ...
      <div className="mt-4 flex items-center justify-between text-xs text-gray-500">
        <span className="capitalize">
          {problem.type === 'multiple_choice' ? 'Multiple Choice' : 'Text Input'}
        </span>
        {!isSubmitted && (
