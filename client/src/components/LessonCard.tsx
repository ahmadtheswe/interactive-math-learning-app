...
interface LessonCardProps {
  lesson: Lesson;
  onLessonClick?: (lessonId: number) => void; // Made optional since we'll handle navigation here
}
...
export default function LessonCard({ lesson, onLessonClick }: LessonCardProps) {
  ...
  const handleClick = () => {
    ...
    // Call the onLessonClick callback if provided (for backward compatibility)
    if (onLessonClick) {
      onLessonClick(lesson.id);
    }

    // Navigate to the lesson with attempt-id query parameter
