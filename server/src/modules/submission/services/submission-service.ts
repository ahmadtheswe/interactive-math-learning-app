...
export class SubmissionService implements ISubmissionService {
  async submitAnswers(
    ...
    try {
      ...
      if (existingSubmissions.length > 0) {
        ...
        const correctAnswersCount = existingSubmissions.filter((s) => s.isCorrect).length;
      ...
      const streakBonusXp = newStreak > 3 ? Math.floor(totalXpAwarded * 0.1) : 0;
  ...
  private async getCorrectAnswersCount(attemptId: string): Promise<number> {
